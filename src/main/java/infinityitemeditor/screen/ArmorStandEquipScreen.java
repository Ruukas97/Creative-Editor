...
public class ArmorStandEquipScreen extends ContainerScreen<EquipmentContainer> {
    ...
    public ArmorStandEquipScreen(Screen lastScreen, TagEntityArmorStand armorStandTag) {
        super(new EquipmentContainer(armorStandTag.getInventory()), Minecraft.getInstance().player.inventory, TextComponent.EMPTY);
    ...
    public void render(PoseStack poseStack,int p_230430_2_, int p_230430_3_, float p_230430_4_) {
        for(int i=0; i<2; i++){
            if(armorStandTag.getHandItems().get()[i] != null){
                entity.setItemSlot(EquipmentSlot.byTypeAndIndex(EquipmentSlot.Group.HAND, i), armorStandTag.getHandItems().get()[i].getItemStack());
            }
        }

        for(int i=0; i<4; i++){
            if(armorStandTag.getArmorItems().get()[i] != null){
                entity.setItemSlot(EquipmentSlot.byTypeAndIndex(EquipmentSlot.Group.ARMOR, i), armorStandTag.getArmorItems().get()[i].getItemStack());
            }
        }
    ...
    protected void renderBg(PoseStack poseStack, float p_230450_2_, int p_230450_3_, int p_230450_4_) {
        ...
        this.minecraft.getTextureManager().bindForSetup(INVENTORY_LOCATION);
        int i = this.leftPos;
        int j = this.topPos;
    ...
    public void renderArmorStand(int x, int y, int scale, float lookX, float lookY) {
        ...
        PoseStack lvt_8_1_ = new PoseStack();

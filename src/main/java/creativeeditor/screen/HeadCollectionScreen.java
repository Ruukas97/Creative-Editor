...
public class HeadCollectionScreen extends ParentScreen {
	...
	public boolean mouseClicked(double mouseX, double mouseY, int mouseButton) {
		...
		if (searchString.length() > 0 && !searchString.equals(filteredString) && GuiUtil.isMouseInRegion((int) mouseX,
				(int) mouseY, (width / 2) - searchW / 2, heightOffset + 6, searchW, 8)) {
			init();
		}

		// Next page
		else if (currentPage + 1 < amountPages && GuiUtil.isMouseInRegion((int) mouseX, (int) mouseY,
				space + letterSpace + maxInRow * 16 - 3 - nextPageW, heightOffset + topbar + 168, nextPageW, 8)) {
			currentElement = Math.min(filteredHeads.size() - 1, (currentPage + 1) * amountInPage);
			playClickSound();
			return false;
		} else if (currentPage > 0 && GuiUtil.isMouseInRegion((int) mouseX, (int) mouseY,
				space + letterSpace + maxInRow * 16 - 25 - nextPageW * 2, heightOffset + topbar + 168, nextPageW, 8)) {
		...
		if (filteredHeads.size() > 0) {
			for (int i = (int) Math.min(filteredHeads.size() - 1, currentPage * amountInPage); i < (int) Math
					.min(filteredHeads.size(), (currentPage + 1) * amountInPage); i++) {
	...
	public void backRender(int mouseX, int mouseY, float p3, Color color) {
		...
		// Background
		fill(space, heightOffset + topbar, space + letterSpace + (maxInRow * 16) + 3, heightOffset + topbar * 2 + 163,
				GuiUtil.getColorFromRGB(110, 0, 0, 0));

		// Letterspace
		fill(space + 2, heightOffset + topbar, space + letterSpace - 2, heightOffset + topbar + 161,
				GuiUtil.getColorFromRGB(100, 50, 50, 50));
		...
		fill(space + letterSpace + maxInRow * 16 + 1, heightOffset + topbar, space + letterSpace + maxInRow * 16 + 3,
				heightOffset + topbar + 161 + topbar, mainColor);
		fill(space + 2, heightOffset + topbar + 161, space + letterSpace + (maxInRow * 16) + 1,
				heightOffset + topbar + 163, mainColor);
		fill(space, heightOffset + topbar * 2 + 161, space + letterSpace + (maxInRow * 16) + 3,
				heightOffset + topbar * 2 + 163, mainColor);

		// Split bar
		fill(space + letterSpace, heightOffset + topbar, space + letterSpace - 2, heightOffset + topbar + 161,
				mainColor);
		...
		for (MinecraftHeadsCategory category : MinecraftHeadsCategory.values()) {
			...
			drawCenteredString(
					font, I18n.format("gui.headcollection.category." + category.getName()), x, y, (style
							.getFGColor(true,
									category == selCat
											|| GuiUtil.isMouseInRegion(mouseX, mouseY, x - sWH, y - 1, sW, 10))
							.getInt()));
		}

		drawString(font, I18n.format("gui.headcollection") + " (" + filteredHeads.size() + ")", space + 7,
				heightOffset + 6, blandColor);

		drawCenteredString(font,
				searchString.length() > 0 ? searchString : I18n.format("gui.headcollection.typesearch"), width / 2,
				heightOffset + 6, blandColor);

		String pageString = I18n.format("gui.headcollection.currentpage", currentPage + 1, amountPages);
		drawString(font, pageString, space + letterSpace + maxInRow * 16 - font.getStringWidth(pageString),
				heightOffset + 6, blandColor);

		String nextPage = "-->";
		int nextPageW = font.getStringWidth(nextPage);
		if (currentPage + 1 < amountPages) {
			boolean selectedN = GuiUtil.isMouseInRegion(mouseX, mouseY,
					space + letterSpace + maxInRow * 16 - 3 - nextPageW, heightOffset + topbar + 168, nextPageW, 8);
			drawString(font, nextPage, space + letterSpace + maxInRow * 16 - 3 - nextPageW, heightOffset + topbar + 168,
					style.getFGColor(true, selectedN).getInt());
		}

		drawCenteredString(font, "" + (currentPage + 1), space + letterSpace + maxInRow * 16 - 13 - nextPageW,
				heightOffset + topbar + 168, blandColor);

		if (currentPage > 0) {
			String previousPage = "<--";
			boolean selectedP = GuiUtil.isMouseInRegion(mouseX, mouseY,
					space + letterSpace + maxInRow * 16 - 25 - nextPageW * 2, heightOffset + topbar + 168, nextPageW,
					8);
			drawString(font, previousPage, space + letterSpace + maxInRow * 16 - 25 - nextPageW * 2,
					heightOffset + topbar + 168, style.getFGColor(true, selectedP).getInt());
		}
		...
		drawCenteredString(font,
				I18n.format("gui.headcollection.freeslots") + ": " + InventoryUtils.getEmptySlots(mc.player.inventory),
				width / 2, height - 45, blandColor);

		drawCenteredString(font,
				I18n.format("gui.headcollection.headsininventory") + ": "
						+ InventoryUtils.countItem(mc.player.inventory, Items.PLAYER_HEAD),
				width / 2, height - 35, blandColor);
		...
		if (hovered != null) {
			GuiUtil.addToolTip(this, mouseX, mouseY, width, height, mouseX, mouseY,
					hovered.getDisplayName().getFormattedText(), "§7" + I18n.format("gui.headcollection.clickhead"),
					"§7" + I18n.format("gui.headcollection.clickheadshift"));
		} else if (!searchString.equals(filteredString)
				&& GuiUtil.isMouseInRegion(mouseX, mouseY, (width / 2) - searchW / 2, 56, searchW, 8)) {
			GuiUtil.addToolTip(this, mouseX, mouseY, I18n.format("gui.headcollection.clicksearch"));
		} else {
			GuiUtil.addToolTip(this, space + 2, heightOffset + topbar, letterSpace - 4, 161, mouseX, mouseY,
					I18n.format("gui.headcollection.changecategory"));
		}

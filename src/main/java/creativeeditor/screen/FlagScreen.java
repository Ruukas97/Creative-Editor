...
public class FlagScreen extends ParentItemScreen {
	...
	protected void init() {
		...
		for (int i = 0; i < amount; i++) {
			...
			addButton(new StyledButton(x, y, 120, 20, I18n.format(HideFlagUtils.Flags.values()[i].getKey()),
					(Button b) -> {
					}));
		}
		addButton(
				new StyledButton(width / 2 - 60, height / 7 * 4, 120, 20, I18n.format("flag.switchall"), (Button b) -> {
				}));
		super.init();
	}
